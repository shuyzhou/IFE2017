...
module.exports = task;
async function task(key,pagination,device) {
    const instance = await phantom.create();
    const page = await instance.createPage();
    let result;
    if(!!device){
    	page.setting('userAgent', option[device].userAgent);
    	page.property('viewportSize', {
            width: option[device].width,
            height: option[device].height,
        })
	};
	const url = `http://www.baidu.com/s?wd=${encodeURIComponent(key)}&pn=${(pagination-1)*10}`
